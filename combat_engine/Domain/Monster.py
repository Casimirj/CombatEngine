...
class Monster:
    ...
    @beartype
    def calc_def_roll(self, attack_type:str=None, combat_style:str=None):
        def_bonus = 0
        level = self.stats.def_level

        if combat_style is not None and combat_style.capitalize() == "Ranged":
            def_bonus = self.stats.ranged_def_light
        elif combat_style is not None and combat_style.capitalize() == "Mage":
            def_bonus = self.stats.magic_def
            level = self.stats.magic_level
        else:
            if attack_type is not None:
                if attack_type.capitalize() == "Slash": def_bonus = self.stats.slash_def
                if attack_type.capitalize() == "Crush": def_bonus = self.stats.crush_def
                if attack_type.capitalize() == "Stab": def_bonus = self.stats.stab_def

        self.def_roll = (level + 9) * (def_bonus + 64)
    ...
    def reduce_magic_level(self, percentage:float):
        """Reduce NPC magic level by given percentage (0-1)."""
        reduction = int(self.stats.magic_level * percentage)
        self.stats.magic_level = max(self.stats.magic_level - reduction, 0)


    def __init__(self, stats:dict=None, weak_to_salve:bool=False, minimum_def:int=0, is_toa_monster:bool=False, in_cox:bool=False):
        if stats == None:
